...
import { backup } from "../../lib/backup-restore/backup";
import { db } from "@vercel/postgres";
import { insertTask } from "@/app/lib/data-model/task-actions";
import { insertEventRecord } from "@/app/lib/data-model/event-actions";
...
async function prep() {
  ...
  await client.sql`DROP TABLE IF EXISTS payments`;
...
async function seedUsers() {
  ...
  const inserted = await Promise.all(
    backup.users.map(async (user) => {
      const hashedPassword = await bcrypt.hash("RunaTuna8288", 10);
      return client.sql`
